...
if (isset($_GET['email'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($_POST["NewUserPassword"] == $_POST["RetypeNewUserPassword"]) {
            ...
            $updatePasswordQuery = "UPDATE admin_password SET Admin_PASSWORD=? WHERE Admin_USER_ID=?";
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                echo '<script>window.location.href = "/Group Assignment/Admin/Admin_ForgotPassword_ChangePasswordPage.php?email=<?php urlencode(base64_encode($email)); ?>";</script>';
            }
            ...
            echo '<script>window.location.href = "/Group Assignment/Admin/Admin_ForgotPassword_ChangePasswordPage.php?email=<?php echo urlencode(base64_encode($email)); ?>";</script>';

        }
    }
} else {
    echo '<script> alert("Email Not Found") </script>'; // Handle the case where the user is not logged in
    echo '<script>window.location.href = "/Group Assignment/Admin/Admin_ForgotPassword_CheckEmail.html";</script>';
}
