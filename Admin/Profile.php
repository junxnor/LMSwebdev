...
function getFirstNameFromDatabase($user_id, $conn) {
    $stmt = $conn->prepare("SELECT Admin_First_Name FROM admin WHERE Admin_USER_ID = ?");
...
function getLastNameFromDatabase($user_id, $conn) {
    $stmt = $conn->prepare("SELECT Admin_Last_Name FROM admin WHERE Admin_USER_ID = ?");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
function getEmailFromDatabase($user_id, $conn) {
    $stmt = $conn->prepare("SELECT Admin_Email FROM admin_email WHERE Admin_USER_ID = ?");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
function getPasswordFromDatabase($user_id, $conn) {
    $stmt = $conn->prepare("SELECT Admin_PASSWORD FROM admin_password WHERE Admin_USER_ID = ?");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
function getGenderFromDatabase($user_id, $conn) {
    $stmt = $conn->prepare("SELECT Gender FROM admin WHERE Admin_USER_ID = ?");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
<body>
    ...
    <div class="Profile">
        ...
        <div class="">
        <button onclick="window.location.href='/Group Assignment/Admin/Admin_CheckSecurityWord.php'">Change Password</button>

        </div>
        <br>
        <br>
        <div class="Back-Button">
        <button onclick="window.location.href='/Group Assignment/Admin/AdminHome.php'">Back to Home Page</button>
        </div>
