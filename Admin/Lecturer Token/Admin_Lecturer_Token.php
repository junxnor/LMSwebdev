...
function getTokenQuestionFromDatabase($user_id, $conn) {
    $stmt = $conn->prepare("SELECT Token_Question FROM token WHERE Instructor_USER_ID = ?");
...
function getTokenNameFromDatabase($user_id, $conn) {
    $stmt = $conn->prepare("SELECT Token_Name FROM token WHERE Instructor_USER_ID = ?");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
function getTokenFilesFromDatabase($user_id, $conn) {
    $stmt = $conn->prepare("SELECT Token_Files FROM token WHERE Instructor_USER_ID = ?");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
function getEmailFromDatabase($user_id, $conn) {
    $stmt = $conn->prepare("SELECT Email FROM token WHERE Instructor_USER_ID = ?");
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
    $result = $stmt->get_result();
...
<body>
    ...
    ?>
        ...
        <form action="/Group Assignment/Admin/Lecturer Token/PHP/Lecturer_Token_Answer.php?user_id=<?php echo $user_id; ?>&token_id=<?php echo $token_id; ?>" method="post">
    <input type="hidden" name="token_id" value="<?php echo $token_id; ?>">
        ...
        <br>
    <div class="Back">
        <button onclick="window.location.href='/Group Assignment/Admin/Lecturer Token/Manage_Lecturer_Token.php'">Back</button>
    </div>
    </div>
</body>
</html>