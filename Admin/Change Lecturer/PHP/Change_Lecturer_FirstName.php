...
if (isset($_GET['user_id'])){
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST["NewFirstName"])) {
            ...
            $updateFirstNameQuery = "UPDATE instructor SET Instructor_FIRST_NAME=? WHERE Instructor_USER_ID=?";
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                echo '<script>window.location.href = "/Group Assignment/Admin/Admin_Lecturer_Profile.php?user_id=' . $user_id . '";</script>';
            } else {
                echo '<script> alert("Error updating first name: ")</script>' . mysqli_stmt_error($stmt);
                echo '<script>window.location.href = "/Group Assignment/Admin/Change Lecturer/Change_Lecturer_FirstName.php?user_id=' . $user_id . '";</script>';
            }
